...
function mostraPergunta() {
    // se não houver mais perguntas, mostrar resultado
    ...
    textoResultado.textContent = ""; // limpa a área de resultado visível
...
function mostraAlternativas() {
    ...
    perguntaAtual.alternativas.forEach((alternativa, index) => {
        ...
        botaoAlternativas.className = "botao-alternativa";
        botaoAlternativas.type = "button";

        // acessibilidade: adicionar atributos
        botaoAlternativas.setAttribute("aria-label", `Alternativa ${index + 1}: ${alternativa.texto}`);
...
function respostaSelecionada(opcaoSelecionada) {
    // adiciona a afirmação selecionada à história final
...
function mostraResultado() {
    ...
    caixaAlternativas.innerHTML = ""; // remove botões
    ...
    intervaloDigitacao = setInterval(() => {
        if (i < historiaFinal.length) {
            textoResultado.textContent += historiaFinal.charAt(i);
            i++;
        } else {
            clearInterval(intervaloDigitacao);
            intervaloDigitacao = null;
            // opcional: adicionar botão para reiniciar o quiz
            criaBotaoReiniciar();
        }
    }, 35);
}
